...
export const getDatabasePoolConfig = () => {
  ...
  const connectionLimit = parseInt(
    process.env.DATABASE_POOL_SIZE || process.env.DATABASE_CONNECTION_LIMIT || String(defaultPoolSize),
    10
  );
...
export const dbPoolConfig = getDatabasePoolConfig();

console.log('📊 Database Pool Configuration:');
console.log(`   Connection Limit: ${dbPoolConfig.connectionLimit}`);
console.log(`   Pool Timeout: ${dbPoolConfig.poolTimeout}s`);
